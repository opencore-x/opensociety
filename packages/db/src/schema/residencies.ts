...
export const residencies = pgTable(
  ...
  // Hot path: resolving which flats a user lives in (auth/ownership checks).
  (t) => [index('residencies_user_id_idx').on(t.userId), index('residencies_apartment_id_idx').on(t.apartmentId)],
)
